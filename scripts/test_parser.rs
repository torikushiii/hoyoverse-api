...
use hoyoverse_api::scraper::sources::starrail::{fandom as starrail_fandom, game8 as starrail_game8};
	...
	let codes: Vec<Code> = match (game.as_str(), source.as_str()) {
		("genshin", "fandom") => {
			...
			genshin_fandom::parse_wikitext(&wikitext)
				.into_iter()
				.map(|c| Code { code: c.code, rewards: c.rewards })
				.collect()
		}
...
fn print_results(codes: &[Code]) {
	...
	println!("{:<30} {}", "CODE", "REWARDS");
	println!("{}", "-".repeat(80));

	for code in codes {
		let normalized = code.code.to_uppercase();
		let dup_marker = if normalized != code.code { " (mixed case)" } else { "" };
	...
	for code in codes {
		seen.entry(code.code.to_uppercase()).or_default().push(code.code.clone());
	}
