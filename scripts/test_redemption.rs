...
fn game_credentials<'a>(
    config: &'a ValidatorConfig,
    slug: &str,
) -> Option<&'a GameValidatorConfig> {
    match slug {
...
async fn main() -> anyhow::Result<()> {
    ...
    if creds.cookie.is_empty() || creds.uid.is_empty() {
        anyhow::bail!("cookie or uid is empty for [{slug}] in config.toml — fill them in first");
    }
    ...
    if resp.is_credentials_issue() {
        println!();
        println!(
            "Hint: check that cookie, uid, and region in config.toml are correct for this game."
        );
    }
