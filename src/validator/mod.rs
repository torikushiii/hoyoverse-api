...
async fn validate_all_codes(global: &Arc<Global>) -> anyhow::Result<()> {
	let all_games = [Game::Genshin, Game::Starrail, Game::Zenless, Game::Honkai, Game::Themis];
	...
	for game in all_games {
		...
		if game.redeem_endpoint().is_none() {
			tracing::warn!(game = game.display_name(), "redeem endpoint not configured, skipping");
			continue;
		}
