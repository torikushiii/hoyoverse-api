use crate::{
    ...
    resolvers::zenless::{
        news::sources::hoyolab::{
            NewsResolver as ZenlessNewsResolver, SUPPORTED_LANGUAGES as ZENLESS_LANGUAGES,
        },
        ZenlessResolver,
    },
                            ...
                            if let Ok(_) = collection.insert_one(insert_doc).await {
                                ...
                                if !is_empty {
                                    ...
                                    if let Err(e) = mutex.acquire(
                                        format!("zenless_code_process:{}", code.code),
                                        || async {
                                            tokio::time::sleep(tokio::time::Duration::from_secs(6)).await;

                                            match verifier.verify_new_code(&code, "zenless").await {
                                                Ok(is_active) => {
                                                    debug!(
                                                        "[Zenless][Codes] Code {} verified: active = {}",
                                                        code.code,
                                                        is_active
                                                    );
                                                }
                                                Err(e) => {
                                                    error!(
                                                        "[Zenless][Codes] Failed to verify code {}: {}",
                                                        code.code,
                                                        e
                                                    );
                                                }
                                        ...
                                        error!("[Zenless][Codes] Mutex error while processing code {}: {}", code.code, e);
                                    }
                                }
                            }
                        }
