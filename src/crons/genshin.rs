                            ...
                            if let Ok(_) = collection.insert_one(insert_doc).await {
                                ...
                                if !is_empty {
                                    ...
                                    if let Err(e) = mutex.acquire(
                                        format!("genshin_code_process:{}", code.code),
                                        || async {
                                            tokio::time::sleep(tokio::time::Duration::from_secs(6)).await;

                                            match verifier.verify_new_code(&code, "genshin").await {
                                                Ok(is_active) => {
                                                    debug!(
                                                        "[Genshin][Codes] Code {} verified: active = {}",
                                                        code.code,
                                                        is_active
                                                    );
                                                }
                                                Err(e) => {
                                                    error!(
                                                        "[Genshin][Codes] Failed to verify code {}: {}",
                                                        code.code,
                                                        e
                                                    );
                                                }
                                        ...
                                        error!("[Genshin][Codes] Mutex error while processing code {}: {}", code.code, e);
                                    }
                                }
                            }
                        }
    ...
    sched.add(Job::new_async("0 */20 * * * *", move |_, _| {
        ...
        Box::pin(async move {
            let mutex = match db.redis.create_mutex().await {
                Ok(mutex) => mutex,
                Err(e) => {
                    error!("[Genshin][News] Failed to create mutex: {}", e);
                    return;
                }
            };

            if let Err(e) = mutex.acquire(
                "genshin_news_fetch".to_string(),
                || async {
                ...
                error!("[Genshin][News] Mutex error: {}", e);
            }
        })
    })?).await?;
