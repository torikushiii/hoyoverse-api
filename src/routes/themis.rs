...
async fn codes(
    ...
    debug!("Handling request for /themis/codes");
    ...
    if !rate_limit.is_allowed() {
        debug!("Rate limit exceeded for /themis/codes");
        return Err(ApiError::rate_limit_exceeded("Too many requests"));
    }

    let cached_data = db.get_cached_data("themis_codes".to_string(), "codes".to_string())
        .await
    ...
    let collection = db.mongo.collection::<GameCode>("themis_codes");
    ...
    if has_codes {
        if let Ok(json) = serde_json::to_string(&response) {
            if let Err(e) = db.redis.set_cached("themis_codes", &json, 300).await {
...
async fn news(
    ...
    let (db, rate_limiter) = state;
    debug!("Handling request for /themis/news/{}", category);
    ...
    let collection = db.mongo.collection::<NewsItem>("themis_news");
