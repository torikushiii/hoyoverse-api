...
use crate::types::{GameCode, CodesResponse, NewsItem};
...
async fn codes(
    ...
    debug!("Handling request for /zenless/codes from {}", addr.ip());
...
async fn news(
    ...
    let cache_key = format!("zenless_news_{}", category);
    let cached_data = db.get_cached_data("zenless_news".to_string(), cache_key.clone()).await;
    if let Ok(Some(data)) = cached_data {
