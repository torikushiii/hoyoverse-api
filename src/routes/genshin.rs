...
async fn codes(
    ...
    debug!("Handling request for /genshin/codes");
    ...
    let cached_data = db.get_cached_data("genshin_codes".to_string(), "codes".to_string())
        .await
...
async fn news(
    ...
    let (db, rate_limiter) = state;
    debug!("Handling request for /genshin/news/{}", category);
