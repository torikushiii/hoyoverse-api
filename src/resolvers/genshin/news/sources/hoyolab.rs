...
impl NewsResolver {
    ...
    pub async fn fetch_news(&self, lang: &str) -> anyhow::Result<Vec<NewsItem>> {
            ...
            .await
        {
            Ok(events) => {
                debug!(
                    "[Genshin] Fetched {} events for language {}",
                    events.list.len(),
                    lang
                );
                for item in events.list {
                    if item.id.is_empty() || item.name.is_empty() {
                        error!(
                            "[Genshin] Skipping event with empty id or name for lang {}",
                            lang
                        );
                        continue;
                    }
            ...
            Err(e) => {
                error!(
                    "[Genshin] Failed to fetch events for language {}: {}",
                    lang, e
                );
            }
        }
        ...
        for (type_id, type_name) in [(1, "notice"), (3, "info")] {
                ...
                .await
            {
                Ok(news) => {
                    ...
                    for item in news.list {
                        if item.post.post_id.is_empty() || item.post.subject.is_empty() {
                            error!(
                                "[Genshin] Skipping {} with empty id or subject for lang {}",
                                type_name, lang
                            );
                            continue;
                        }
                ...
                Err(e) => {
                    error!(
                        "[Genshin] Failed to fetch {} for language {}: {}",
                        type_name, lang, e
                    );
                }
            }
        ...
        debug!(
            "[Genshin] Total news items fetched for {}: {}",
            lang,
            all_news.len()
        );
        Ok(all_news)
...
pub async fn fetch_news(config: &Settings, _category: &str) -> anyhow::Result<Vec<NewsItem>> {
    ...
    for lang in SUPPORTED_LANGUAGES {
        match resolver.fetch_news(lang).await {
            ...
            Err(e) => error!(
                "[Genshin] Failed to fetch news for language {}: {}",
                lang, e
            ),
        }
    }
