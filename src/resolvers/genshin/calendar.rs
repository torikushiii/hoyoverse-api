...
pub async fn fetch_calendar(
    ...
    let response = client.post(CALENDAR_URL).json(&body).send().await?;

    let response_text = response.text().await?;

    let calendar: GenshinCalendarResponse = serde_json::from_str(&response_text).map_err(|e| {
        error!(
    ...
    for event in data.act_list {
        ...
        let (image_url, db_start_time, db_end_time) = get_event_data(mongo, &event.name)
            .await
            .unwrap_or((String::new(), None, None));

        let final_start_time = if start_time == 0 {
            db_start_time.unwrap_or(0)
        } else {
            start_time
        };
        let final_end_time = if end_time == 0 {
            db_end_time.unwrap_or(0)
        } else {
            end_time
        };
