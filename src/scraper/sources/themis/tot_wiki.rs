...
pub fn parse_html(html: &str) -> Vec<ParsedCode> {
	...
	for cap in row_re.captures_iter(table_html) {
		...
		for code in code_field.split(',').map(|s| s.trim().to_uppercase()).filter(|s| !s.is_empty()) {
			results.push(ParsedCode { code, rewards: rewards.clone() });
		}
	}
...
pub async fn scrape_and_store(global: &Arc<Global>) -> anyhow::Result<usize> {
	...
	tracing::info!(new = new_count, total = scraped.len(), "tot_wiki scrape complete");
