...
pub fn parse_html(html: &str) -> Vec<ParsedCode> {
	...
	let fallback_code_re = Regex::new(r"gift\?code=([A-Z0-9]{8,})").expect("invalid fallback code regex");
	let reward_re =
		Regex::new(r#"(?s)<div class="align">.*?<a.*?>(.*?)</a>\s*(x[\d,]+)"#).expect("invalid reward regex");
