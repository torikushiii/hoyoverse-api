...
pub fn parse_html(html: &str) -> Vec<ParsedCode> {
	...
	let row_re = Regex::new(r"(?s)<tr>\s*<td.*?>(.*?)</td>\s*<td.*?>(.*?)</td>").expect("invalid row regex");
	let code_re = Regex::new(r#"value=['"]([^'"]+)['"]"#).expect("invalid code regex");
	let fallback_code_re = Regex::new(r"redemption\?code=([A-Za-z0-9]+)").expect("invalid fallback code regex");
	let reward_re =
		Regex::new(r#"(?s)<div class=['"]align['"]>.*?<a.*?>(.*?)</a>\s*x?\s*([\d,]+)"#).expect("invalid reward regex");
...
pub async fn scrape_and_store(global: &Arc<Global>) -> anyhow::Result<usize> {
	...
	tracing::info!(new = new_count, total = scraped.len(), "game8 scrape complete");
